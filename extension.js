  ...
  class ClaudeUsageIndicator extends PanelMenu.Button {
    _init(extensionPath, settings, openPreferences) {
      ...
      this.menu.connect("open-state-changed", (_menu, open) => {
        if (open && Date.now() - this._lastFetchTime > 60000)
          this._refreshUsage();
      });
    ...
    _createSession() {
      ...
      if (proxyUrl && proxyUrl.trim() !== "") {
        const proxyResolver = Gio.SimpleProxyResolver.new(
          proxyUrl.trim(),
          null,
        );
    ...
    _updateIconStyle() {
      ...
      if (style === "monochrome" && !hasEffect) {
        this._icon.add_effect(
          new Clutter.DesaturateEffect({ factor: 1.0, name: desatName }),
        );
          ...
          try {
            ...
            if (message.status_code === 429) {
              ...
              const msUntilNextHour =
                ...
                now.getSeconds() * 1000 -
                now.getMilliseconds();
              const delaySecs = Math.max(Math.ceil(msUntilNextHour / 1000), 60);
              if (this._lastData) {
                this._updateDisplay(this._lastData);
              }
              this._fiveHourResetLabel.set_text(
                "Rate limited — retrying next hour",
              );
              this._scheduleRetry(delaySecs);
              return;
            }
    ...
    _updatePanelProgressBar(usage) {
      ...
      const width = Math.round(
        (Math.min(100, Math.max(0, usage)) / 100) * maxWidth,
      );
...
export default class ClaudeUsageExtension extends Extension {
  enable() {
    ...
    this._indicator = new ClaudeUsageIndicator(this.path, this._settings, () =>
